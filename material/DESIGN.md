...
/plugin auth login|callback|status|logout ...
```

- `/plugin` 每個 subcommand 都直接比對 caller ID 與 `config.discord.owner_id`，不接受 guild role 或 channel allowlist 代替 owner 身分。所有回覆用 ephemeral；耗時操作先 defer interaction。Discord HTTPS source 禁止內嵌 username/password/token；私有 Gitea 預設走 `/plugin auth login` + 手動 callback URL 回填的 OAuth PKCE 流程，SSH 只保留為手動 fallback。
...
- 私有 Gitea OAuth 不要求 Furet 對外提供 callback server：`/plugin auth login` 產生 `state`、PKCE verifier 與授權 URL；瀏覽器授權後即使 loopback redirect 顯示連線失敗，使用者仍可把網址列中的完整 callback URL 貼回 `/plugin auth callback`。Furet 只解析其中的 `code`／`state`，再由 server 呼叫 token endpoint，不會 GET 該 loopback URL。
