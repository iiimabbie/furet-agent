...
- `/plugin` — owner-only 單一入口；`source` 安裝、無參數列出、`auth`／`callback` 授權，低頻管理走 `action` option
...
- 私有 Gitea OAuth 不要求 Furet 對外提供 callback server：`/plugin auth:<host>` 產生 `state`、PKCE verifier 與授權 URL；瀏覽器授權後即使 loopback redirect 顯示連線失敗，使用者仍可把網址列中的完整 callback URL 貼回 `/plugin callback:<url>`。Furet 只解析其中的 `code`／`state`，再由 server 呼叫 token endpoint，不會 GET 該 loopback URL。
