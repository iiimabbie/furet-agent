...
await context.messages.editText({
  channelId: "CHANNEL_ID",
...
The host sends standard Discord V1 text messages and rejects an edit unless the target message belongs to the bot. This transport is for plugin-owned, persisted workflows that must deliver exact generated bytes without making the agent reserialize them. It is not a public agent tool and does not expose arbitrary Discord client access.
