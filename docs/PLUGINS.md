...
The host sends Components V2 text messages and rejects an edit unless the target message belongs to the bot. This transport is for plugin-owned, persisted workflows that must deliver exact generated bytes without making the agent reserialize them. It is not a public agent tool and does not expose arbitrary Discord client access.
